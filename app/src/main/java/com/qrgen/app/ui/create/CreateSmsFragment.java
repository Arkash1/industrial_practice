...
import com.qrgen.app.databinding.FragmentCreateSmsBinding;
import com.qrgen.app.generator.QRContentBuilder;

public class CreateSmsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        binding = FragmentCreateSmsBinding.inflate(inflater, container, false);
    ...
    private void setupUI() {
        ...
        binding.buttonGenerate.setOnClickListener(v -> {
            ...
            if (phone.isEmpty()) {
                binding.inputLayoutPhone.setError("Введите номер телефона");
                return;
            }
            binding.inputLayoutPhone.setError(null);
            try {
                String content = QRContentBuilder.buildSmsContent(phone, message);
                viewModel.generateQRCode(content, QRType.SMS.getValue(), "SMS: " + phone);
            } catch (IllegalArgumentException e) {
                binding.inputLayoutPhone.setError(e.getMessage());
            }
        });
    ...
    private void observeViewModel() {
        viewModel.getGeneratedQR().observe(getViewLifecycleOwner(), bitmap -> {
            if (bitmap != null) {
                ...
                args.putString("type", QRType.SMS.getValue());
                ...
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_createSms_to_result, args);
            }
        });
        ...
        viewModel.getIsLoading().observe(getViewLifecycleOwner(), loading -> {
            ...
            binding.buttonGenerate.setEnabled(!loading && binding.editTextPhone.getText().length() > 0);
        });
    }
