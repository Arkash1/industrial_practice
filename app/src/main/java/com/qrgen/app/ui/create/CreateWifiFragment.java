...
import com.qrgen.app.databinding.FragmentCreateWifiBinding;
import com.qrgen.app.generator.QRContentBuilder;

public class CreateWifiFragment extends Fragment {

    private FragmentCreateWifiBinding binding;
    ...
    private void setupUI() {
        binding.radioGroupEncryption.setOnCheckedChangeListener((group, checkedId) -> {
            boolean needPassword = checkedId != R.id.radioNone;
            binding.inputLayoutPassword.setVisibility(needPassword ? View.VISIBLE : View.GONE);
        });

        binding.buttonGenerate.setOnClickListener(v -> {
            String ssid = binding.editTextSsid.getText().toString().trim();
            String password = binding.editTextPassword.getText().toString();
            boolean hidden = binding.checkboxHidden.isChecked();

            if (ssid.isEmpty()) {
                binding.inputLayoutSsid.setError("Введите имя сети");
                return;
            }
            binding.inputLayoutSsid.setError(null);

            String encryptionType;
            int checkedId = binding.radioGroupEncryption.getCheckedRadioButtonId();
            if (checkedId == R.id.radioWpa) {
                encryptionType = "WPA";
            } else if (checkedId == R.id.radioWep) {
                encryptionType = "WEP";
            } else {
                encryptionType = "nopass";
            }

            if (!"nopass".equals(encryptionType) && password.isEmpty()) {
                binding.inputLayoutPassword.setError("Введите пароль");
                return;
            }
            binding.inputLayoutPassword.setError(null);

            String content = QRContentBuilder.buildWifiContent(ssid, password, encryptionType, hidden);
            viewModel.generateQRCode(content, QRType.WIFI.getValue(), "Wi-Fi: " + ssid);
        });
    ...
    private void observeViewModel() {
        viewModel.getGeneratedQR().observe(getViewLifecycleOwner(), bitmap -> {
            if (bitmap != null) {
                ...
                args.putString("type", QRType.WIFI.getValue());
                ...
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_createWifi_to_result, args);
            }
        });
