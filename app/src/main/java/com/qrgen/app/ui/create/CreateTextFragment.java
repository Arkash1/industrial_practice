...
import com.qrgen.app.databinding.FragmentCreateTextBinding;
import com.qrgen.app.generator.QRContentBuilder;

public class CreateTextFragment extends Fragment {

    private FragmentCreateTextBinding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        binding = FragmentCreateTextBinding.inflate(inflater, container, false);
    ...
    private void setupUI() {
        binding.editTextContent.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                int length = s.length();
                binding.textCharCount.setText(length + "/" + QRContentBuilder.getMaxContentLength());
                binding.buttonGenerate.setEnabled(length > 0 && length <= QRContentBuilder.getMaxContentLength());

                if (length > QRContentBuilder.getMaxContentLength()) {
                    binding.inputLayoutContent.setError("Превышен лимит символов");
                } else {
                    binding.inputLayoutContent.setError(null);
                }
            }
        ...
        binding.buttonGenerate.setOnClickListener(v -> {
            ...
            if (!text.trim().isEmpty()) {
                String content = QRContentBuilder.buildTextContent(text);

                if (viewModel.hasLowContrast()) {
                    Toast.makeText(getContext(),
                            "Предупреждение: низкий контраст цветов может затруднить сканирование",
                            Toast.LENGTH_LONG).show();
                }

                viewModel.generateQRCode(content, QRType.TEXT.getValue(), text);
            }
        });
    ...
    private void observeViewModel() {
        viewModel.getGeneratedQR().observe(getViewLifecycleOwner(), bitmap -> {
            if (bitmap != null) {
                ...
                args.putString("type", QRType.TEXT.getValue());
                ...
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_createText_to_result, args);
            }
        });
        ...
        viewModel.getIsLoading().observe(getViewLifecycleOwner(), loading -> {
            ...
            binding.buttonGenerate.setEnabled(!loading &&
                    binding.editTextContent.getText().length() > 0);
        });
    }
